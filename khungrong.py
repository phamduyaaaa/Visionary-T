...
class TransparentWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.border_opacity = None
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint)
        self.setAttribute(Qt.WA_TranslucentBackground)

        # Hiển thị chỉ nửa màn hình dọc bên trái
        screen_width = 2000  # Chiều rộng màn hình
    ...
    def mousePressEvent(self, event):
        ...
        resize_edge = self.detect_resize_edge(event.pos())
        if resize_edge:
    ...
    def mouseMoveEvent(self, event):
        if self._is_resizing:
            ...
            min_width, min_width = 50, 50  # Kích thước tối thiểu
            ...
            new_height = max(min_width, min(new_height, max_height))
...
if __name__ == "__main__":
    ...
    window.set_border_opacity(255)  # Đặt độ đục của viền 
    sys.exit(app.exec_())
